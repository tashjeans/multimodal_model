#!/usr/bin/env python3
"""Export LoRA-finetuned ESMC-300m per-residue embedding shards.

Standalone CLI mirroring ``scripts/preprocess/export_raw_esmc_embedding_shards.py``,
but loads the three LoRA modality encoders from checkpoints instead of a single
frozen ESMC.

Pipeline
--------
CSV strings -> ESMC tokenizer (clean, unmasked) -> LoRA ESMC forward -> shards

Each ``shard_XXXXX.pt`` is a list of batch dicts with keys:
    emb_T, emb_P, emb_H, mask_T, mask_P, mask_H,
    binding_flag, pair_id, batch_idx, split

Typical launch (overwrite val/test only; leave train untouched)
--------------------------------------------------------------
    cd /home/natasha/multimodal_model
    conda activate tcr-multimodal
    python scripts/train/LoRA_esmc_embedding_out.py \\
        --embed-root models/embeddings/no_boltz_multiview_ids \\
        --splits val,test \\
        --val-csv data/val/val_multiview.csv \\
        --test-csv data/test/test_multiview.csv \\
        --clear-existing

KNN train (full positives + matched negatives)
----------------------------------------------
    python scripts/train/LoRA_esmc_embedding_out.py \\
        --embed-root models/embeddings/KNN_embeddings_dir \\
        --splits train \\
        --train-csv data/train/train_multiview_negative_decoys.csv \\
        --clear-existing
"""
...
class ExportConfig:
    ...
    embed_root: str = str(REPO / "models/embeddings/no_boltz_multiview_ids")
