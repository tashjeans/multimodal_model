...
def per_peptide_macro_auroc(labels, scores, pair_ids, pep_lookup):
    rows = []
    for y, s, pid in zip(labels, scores, pair_ids):
        pep = pep_lookup.get(str(pid), None)
        if pep is None:
            continue
        rows.append({"peptide": pep, "label": int(y), "score": float(s)})
    if len(rows) == 0:
        return {"macro": float("nan"), "weighted": float("nan"), "n_peptides_total": 0, "n_peptides_valid": 0}
    ...
    for pep, grp in df.groupby("peptide", sort=True):
        ...
        per.append({"peptide": pep, "n": int(len(grp)), "valid": bool(valid), "auroc": auc})
    per_df = pd.DataFrame(per)
    valid_df = per_df[per_df["valid"]]
    if len(valid_df) == 0:
        macro = float("nan")
        weighted = float("nan")
    else:
        macro = float(valid_df["auroc"].mean())
        weighted = float(np.average(valid_df["auroc"], weights=valid_df["n"]))
    return {
        "macro": macro,
        "weighted": weighted,
        "n_peptides_total": int(len(per_df)),
        "n_peptides_valid": int(len(valid_df)),
    }


def boltz_only_scores_from_pair_ids(pair_ids, latent_store):
    scores = []
    kept = []
    for pid in pair_ids:
        lat = latent_store.get(pid)
        if lat is None:
            continue
        v = np.asarray(lat, dtype=np.float32)
        scores.append(float(np.linalg.norm(v)))
        kept.append(pid)
    return np.asarray(scores, dtype=np.float32), kept

# ============================================================
# LOGGING
# ============================================================
LOG_FILE = "hpo_full_hamiltonian_boltz_immrepA_alpha1.log"
logging.basicConfig(
    level=logging.INFO,
...
VAL_CSV   = str(PROJECT / "data/val/val_df_clean_pos_neg_A.csv")
TEST_CSV  = str(PROJECT / "data/test/test_df_clean_pos_neg_A.csv")

BOLTZ_LATENT_DIR = PROJECT / "models/embeddings/boltz_signal_bottleneck_latents"
ZSTAR_D = 128
ZSTAR_RANK = 8

FIGURE_SUBDIR = FIGURE_DIR / "hpo_training/hpo_full_hamiltonian_boltz_immrepA_alpha1"
FIGURE_SUBDIR.mkdir(parents=True, exist_ok=True)
SAVE_DIR = CHECKPOINTS_DIR / "hpo_training/hpo_full_hamiltonian_boltz_immrepA_alpha1"
...
SEARCH_SPACE = [
    {"rL": 8, "rD": 16, "lr": 1e-4, "wd": 1e-2, "alpha": 1.0, "beta": 25.0, "delta": 1.0, "lambda_ang": 25.0, "gamma_ang": 0.05},
]
NUM_EPOCHS = 20
PATIENCE = 5
D = 128
...
def vicreg_hamiltonian_loss(
    ...
    L_ang = (
        F.relu(gamma_ang - torch.sqrt((eT - eT.mean(dim=0, keepdim=True)).var(dim=0, unbiased=False) + eps)).mean()
        + F.relu(gamma_ang - torch.sqrt((ePH - ePH.mean(dim=0, keepdim=True)).var(dim=0, unbiased=False) + eps)).mean()
    )

    # ---- 4) Total ----
    L_total = alpha * L_inv + beta * L_var_total + delta * L_cov_total + lambda_ang * L_ang
    ...
    components = {
        ...
        "lambda_ang_term": (lambda_ang * L_ang).item(),
        "L_ang": L_ang.item(),
...
@torch.no_grad()
def forward_batch_no_boltz(batch, tcr_proj, pmhc_proj, device, eps=1e-8):
    eT = batch["emb_T"].to(device); mT = batch["mask_T"].to(device)
    eP = batch["emb_P"].to(device); mP = batch["mask_P"].to(device)
    eH = batch["emb_H"].to(device); mH = batch["mask_H"].to(device)
    zT = tcr_proj(eT, mT); zPH = pmhc_proj(eP, mP, eH, mH)
    ...
    labels = labels.cpu().numpy() if torch.is_tensor(labels) else np.array(labels)
    return {
        "zT": zT, "zPH": zPH, "cos": cos.cpu().numpy(), "H": H.cpu().numpy(),
        "score": (-H).cpu().numpy(), "labels": labels, "pair_ids": batch["pair_id"],
    }
...
@torch.no_grad()
def evaluate_loader_no_boltz(loader, tcr_proj, pmhc_proj, device, eps=1e-8):
    tcr_proj.eval(); pmhc_proj.eval()
    all_scores, all_H, all_cos, all_lab, all_pid = [], [], [], [], []
    for batch in loader:
        out = forward_batch_no_boltz(batch, tcr_proj, pmhc_proj, device, eps)
        all_scores.append(out["score"]); all_H.append(out["H"]); all_cos.append(out["cos"])
        all_lab.append(out["labels"]); all_pid.extend(out["pair_ids"])
    scores = np.concatenate(all_scores); H_vals = np.concatenate(all_H); cos_vals = np.concatenate(all_cos)
    labels = np.concatenate(all_lab).astype(int)
    thr = find_best_threshold(scores, labels)
    return {
        "scores": scores, "H": H_vals, "cos": cos_vals, "labels": labels, "pair_ids": all_pid,
        "metrics": {
            "auroc": roc_auc_score(labels, scores),
            "auprc": average_precision_score(labels, scores),
            "score_type": "no_boltz_hamiltonian",
            **thr,
        },
    }
...
def run_experiment(train_loader, val_loader, device, pep_lookup_val,
                   ...
                   alpha=1.0, beta=25.0, delta=1.0, lambda_ang=25.0, gamma_var=1.0, gamma_ang=0.05,
                   run_tag="cfg0"):
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        for batch in train_loader:
            ...
            loss, _ = vicreg_hamiltonian_loss(
                zT, zPH, Zstar,
                alpha=alpha, beta=beta, delta=delta, lambda_ang=lambda_ang, gamma_var=gamma_var, gamma_ang=gamma_ang
            )
        ...
        val_out = evaluate_loader_no_boltz(val_loader, tcr_proj, pmhc_proj, device)
        vl = float("nan"); va = val_out["metrics"]["auroc"]
        vp = val_out["metrics"]["auprc"]; vf = val_out["metrics"]["f1"]
        va_cos = float("nan"); va_H = va
        pep_auc = per_peptide_macro_auroc(val_out["labels"], val_out["scores"], val_out["pair_ids"], pep_lookup_val)
        ...
        history.setdefault("val_auroc_per_peptide_macro", []).append(pep_auc["macro"])
        log.info(f"  [{run_tag}] ep{epoch+1}/{NUM_EPOCHS} tl={tl:.4f} "
                 f"val_auroc={va:.4f}({val_out['metrics']['score_type']}) "
                 f"val_auroc_pep_macro={pep_auc['macro']:.4f} "
                 f"auprc={vp:.4f} f1={vf:.4f}")
        ...
        if va > best_auroc + 1e-4:
            ...
            best_state = {
                ...
                "val_auroc_per_peptide_macro": pep_auc["macro"],
                "config": {"rL": rL, "rD": rD, "d": D, "R_PH": R_PH, "lr": lr,
                           "weight_decay": weight_decay, "alpha": alpha, "beta": beta,
                           "delta": delta, "lambda_ang": lambda_ang, "gamma_ang": gamma_ang},
            }
            torch.save({
...
def main():
    ...
    # Load Boltz latents and create paired training dataset.
    # Validation/test deliberately exclude Boltz for score comparability.
    ...
    for ci, cfg in enumerate(SEARCH_SPACE):
        ...
        result = run_experiment(
            ...
            delta=cfg["delta"], lambda_ang=cfg["lambda_ang"], gamma_ang=cfg["gamma_ang"],
            run_tag=tag,
        )
        ...
        all_results.append(
            {
                "config": cfg,
                "auroc": va,
                "val_auroc_per_peptide_macro": result["best_state"].get("val_auroc_per_peptide_macro", float("nan")),
    ...
    test_out = evaluate_loader_no_boltz(test_loader, best_result["tcr_proj"], best_result["pmhc_proj"], device)
    preds = (test_out["scores"] >= best_thr).astype(int)
    labels = test_out["labels"]
    test_pep_auc = per_peptide_macro_auroc(labels, test_out["scores"], test_out["pair_ids"], pep_lookup_test)
    val_boltz_scores, val_boltz_pids = boltz_only_scores_from_pair_ids(
        best_result["best_state"]["val_outputs"]["pair_ids"], op_val
    )
    val_boltz_labels = np.array(
        [int(y) for y, pid in zip(best_result["best_state"]["val_outputs"]["labels"], best_result["best_state"]["val_outputs"]["pair_ids"]) if op_val.has(pid)],
        dtype=int,
    )
    # choose orientation on validation
    val_boltz_pep_pos = per_peptide_macro_auroc(val_boltz_labels, val_boltz_scores, val_boltz_pids, pep_lookup_val)["macro"]
    val_boltz_pep_neg = per_peptide_macro_auroc(val_boltz_labels, -val_boltz_scores, val_boltz_pids, pep_lookup_val)["macro"]
    boltz_sign = 1.0 if (np.isnan(val_boltz_pep_neg) or val_boltz_pep_pos >= val_boltz_pep_neg) else -1.0
    test_boltz_scores, test_boltz_pids = boltz_only_scores_from_pair_ids(test_out["pair_ids"], op_test)
    test_boltz_labels = np.array(
        [int(y) for y, pid in zip(test_out["labels"], test_out["pair_ids"]) if op_test.has(pid)],
        dtype=int,
    )
    test_boltz_pep_auc = per_peptide_macro_auroc(
        test_boltz_labels, boltz_sign * test_boltz_scores, test_boltz_pids, pep_lookup_test
    )
    log.info(f"\nTest AUROC: {test_out['metrics']['auroc']:.4f}")
    log.info(f"Test AUPRC: {test_out['metrics']['auprc']:.4f}")
    log.info(f"Test per-peptide macro AUROC (trained no-boltz score): {test_pep_auc['macro']:.4f}")
    log.info(f"Val per-peptide macro AUROC (boltz-only baseline): {max(val_boltz_pep_pos, val_boltz_pep_neg):.4f}")
    log.info(f"Test per-peptide macro AUROC (boltz-only baseline): {test_boltz_pep_auc['macro']:.4f}")
