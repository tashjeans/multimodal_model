...
9. Uses VICReg-style covariance scaling: sum(offdiag(C)^2) / d, not / d^2.
...
def group_variance_floor(z, group_ids, gamma=0.05, min_group_size=3):
    """Peptide-aware variance floor: compatible TCRs should not become identical."""
    if group_ids is None:
        return z.new_tensor(0.0), {
            "group_var_loss": 0.0,
            "group_var_n_groups": 0,
            "group_var_std_mean": 0.0,
        }
...
def build_group_lookup(csv_path, id_col):
    ...
    if id_col not in df.columns:
        raise ValueError(f"{id_col} missing from {csv_path}. Run the ID-generation cell first.")
...
@torch.no_grad()
def evaluate_loader(loader, tcr_proj, pmhc_proj, device, cfg, overlap_bins=60, keep_embeddings=False, peptide_pair_to_code=None):
    tcr_proj.eval()
    pmhc_proj.eval()
    ...
    stat_acc = {
        "H_std": [], "cos_std": [], "zT_dim_std_mean": [], "zPH_dim_std_mean": [],
        "L_inv": [], "L_var_total": [], "L_cov_total": [],
        "L_peptide_group": [], "group_var_loss": [], "group_var_n_groups": [], "group_var_std_mean": []
    }
...
# TRAINING
# ============================================================
def run_single(cfg, train_loader, val_loader, device, dirs, overlap_bins, guard_cfg, train_peptide_lookup=None, val_peptide_lookup=None):
    ...
    for epoch in range(cfg["max_epochs"]):
        ...
        log.info(
            ...
            f"zTstd={epoch_row['zT_dim_std_mean']:.4f} | grp_std={epoch_row.get('group_var_std_mean', 0.0):.4f} | "
...
def final_evaluate(best_state, train_loader, val_loader, test_loader, device, overlap_bins, val_peptide_lookup=None, test_peptide_lookup=None):
    tcr_proj, pmhc_proj = load_best_models(best_state, train_loader, device)
    cfg = best_state["cfg"]

    val_out = evaluate_loader(val_loader, tcr_proj, pmhc_proj, device, cfg, overlap_bins=overlap_bins, keep_embeddings=True, peptide_pair_to_code=val_peptide_lookup)
    val_thr = find_best_threshold_on_H(val_out["H"], val_out["labels"])

    test_out = evaluate_loader(test_loader, tcr_proj, pmhc_proj, device, cfg, overlap_bins=overlap_bins, keep_embeddings=True, peptide_pair_to_code=test_peptide_lookup)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--config",
        type=str,
        default="/home/natasha/multimodal_model/scripts/train/hpo_seed31_groupvar_config_old.json",
