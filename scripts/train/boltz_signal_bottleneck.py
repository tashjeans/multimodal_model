#!/usr/bin/env python3
"""
Signal-aware structured Boltz bottleneck operator model
======================================================
...
5. Explicitly optimise for class separation as well as feature preservation.
6. Save intermediate checkpoints and preview exports during training.
...
- binding classification from reconstructed Z* features
- latent spread/decorrelation
- Z* scale control

Run in tmux
-----------
tmux new -s boltz_signal
...
class Config:
    ...
    lambda_bind_latent: float = 0.50
    lambda_bind_zstar: float = 1.00
...
class SignalAwareOperatorBottleneck(nn.Module):
    ...
    def __init__(
        ...
        self.zstar_binding_head = nn.Sequential(
            nn.Linear(target_feature_dim, 64),
            nn.GELU(),
            nn.Dropout(dropout),
            nn.Linear(64, 1),
        )
    ...
    def forward(self, z_boltz: torch.Tensor, orig_len: torch.Tensor):
        ...
        zstar_binding_logit = self.zstar_binding_head(zstar_feat_norm).squeeze(-1)
        ...
        return {
            ...
            "zstar_binding_logit": zstar_binding_logit,
        }
...
def compute_losses(model, out, batch):
    ...
    bind_latent_loss = F.binary_cross_entropy_with_logits(out["binding_logit"], bind_target)
    bind_zstar_loss = F.binary_cross_entropy_with_logits(out["zstar_binding_logit"], bind_target)
    ...
    total = (
        ...
        + CFG.lambda_bind_latent * bind_latent_loss
        + CFG.lambda_bind_zstar * bind_zstar_loss
    ...
    return {
        ...
        "bind_latent": bind_latent_loss,
        "bind_zstar": bind_zstar_loss,
        "var": reg_terms["var"],
        "cov": reg_terms["cov"],
        "theta_l2": theta_l2,
...
def run_epoch(model, loader, optimizer, scheduler, device, scaler, train: bool):
    ...
    pbar = tqdm(loader, desc=("train" if train else "val  "), ncols=150, miniters=20)
    if train:
    ...
    for step, batch in enumerate(pbar):
        ...
        for k in ["latent_feat", "zstar_feat", "bind_latent", "bind_zstar", "var", "cov", "theta_l2", "zstar_scale"]:
...
def main():
    ...
    with open(os.path.join(CFG.checkpoint_dir, "boltz_signal_bottleneck_config.json"), "w") as f:
    ...
    current_ckpt_path = os.path.join(CFG.checkpoint_dir, "current_boltz_signal_bottleneck.pt")
    best_ckpt_path = os.path.join(CFG.checkpoint_dir, "best_boltz_signal_bottleneck.pt")

    preview_loader = {"train": train_loader, "val": val_loader, "test": test_loader}[CFG.preview_split]

    print(f"\n=== Signal-aware bottleneck training: up to {CFG.max_epochs} epochs ===")
    ...
    for epoch in range(1, CFG.max_epochs + 1):
        ...
        history_row = {
            ...
            "train_bind_latent": train_m["bind_latent"],
            "train_bind_zstar": train_m["bind_zstar"],
            ...
            "val_bind_latent": val_m["bind_latent"],
            "val_bind_zstar": val_m["bind_zstar"],
            "val_var": val_m["var"],
            "val_cov": val_m["cov"],
        ...
        print(
            ...
            f"train={train_m['loss']:.4f} latent={train_m['latent_feat']:.4f} zstar={train_m['zstar_feat']:.4f} "
            f"bind_lat={train_m['bind_latent']:.4f} bind_z={train_m['bind_zstar']:.4f} scale={train_m['zstar_scale']:.4f} | "
            f"val={val_m['loss']:.4f} latent={val_m['latent_feat']:.4f} zstar={val_m['zstar_feat']:.4f} "
            f"bind_lat={val_m['bind_latent']:.4f} bind_z={val_m['bind_zstar']:.4f} scale={val_m['zstar_scale']:.4f} | "
        ...
        if CFG.save_epoch_checkpoints:
            epoch_ckpt_path = os.path.join(CFG.checkpoint_dir, f"epoch_{epoch:03d}_boltz_signal_bottleneck.pt")
        ...
        history_path = os.path.join(CFG.checkpoint_dir, "boltz_signal_bottleneck_history.csv")
