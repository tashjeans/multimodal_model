#!/usr/bin/env python3
"""
Simplified VICReg-Hamiltonian diagnostic run on strict TULIP-style decoys.
...
- validation/test Hamiltonian histograms for model and raw ESM
...
def angular_variance(e: torch.Tensor, gamma_ang: float = 0.04, eps_var: float = 1e-4) -> torch.Tensor:
    e = e - e.mean(dim=0, keepdim=True)
    std = torch.sqrt(e.var(dim=0, unbiased=False) + eps_var)
...
def vicreg_hamiltonian_loss(
    ...
    L_ang = angular_variance(eT, gamma_ang, eps_var) + angular_variance(ePH, gamma_ang, eps_var)
    ...
    parts = {
        ...
        "weighted_ang": float((lambda_ang * L_ang).detach().cpu()),
...
def evaluate(
    ...
    running = {k: 0.0 for k in [
        "loss", "L_inv", "L_var", "L_cov", "L_ang",
        "weighted_inv", "weighted_var", "weighted_cov", "weighted_ang",
...
class RunConfig:
    ...
    out_dir: str = "/home/natasha/multimodal_model/models/checkpoints/hpo_training/vicreg_hamiltonian_tulip_simple"
    fig_dir: str = "/home/natasha/multimodal_model/models/figures/hpo_training/vicreg_hamiltonian_tulip_simple"
    run_tag: str = "tulip_decoys_simple"
    ...
    beta: float = 25.0
    delta: float = 1.0
    lambda_ang: float = 5.0
    gamma_var: float = 1.0
    gamma_ang: float = 0.04
    eps_norm: float = 1e-8
    eps_var: float = 1e-4
...
def main() -> None:
    ...
    parser.add_argument("--lambda-ang", type=float, default=RunConfig.lambda_ang)
    parser.add_argument("--gamma-ang", type=float, default=RunConfig.gamma_ang)
    ...
    print("============================================================", flush=True)
    print("Simplified VICReg-Hamiltonian diagnostic run", flush=True)
    ...
    print(f"Loss: alpha={cfg.alpha}, beta={cfg.beta}, delta={cfg.delta}, lambda_ang={cfg.lambda_ang}, gamma_ang={cfg.gamma_ang}", flush=True)
    ...
    for epoch in range(1, cfg.epochs + 1):
        ...
        train_running = {k: 0.0 for k in ["loss", "L_inv", "L_var", "L_cov", "L_ang", "weighted_inv", "weighted_var", "weighted_cov", "weighted_ang"]}
        n = 0
        ...
        for batch in train_loader:
            ...
            loss, parts = vicreg_hamiltonian_loss(zT, zPH, **lp, return_parts=True)
        ...
        row = {
            ...
            "train_L_ang": train_running["L_ang"] / max(1, n),
            ...
            "train_weighted_ang": train_running["weighted_ang"] / max(1, n),
            **val_eval["metrics"],
        }
        history.append(row)
        ...
        print(
            ...
            f"w_ang={row['train_weighted_ang']:.4f} | "
    ...
    plot_histogram(val_eval["model_H"], val_eval["labels"], "Validation model Hamiltonian", fig_dir / f"{stem}__val_model_H_hist.png")
    plot_histogram(val_eval["raw_H"], val_eval["labels"], "Validation raw ESM proxy Hamiltonian", fig_dir / f"{stem}__val_raw_esm_H_hist.png")
    plot_histogram(test_eval["model_H"], test_eval["labels"], "Test model Hamiltonian", fig_dir / f"{stem}__test_model_H_hist.png")
    plot_histogram(test_eval["raw_H"], test_eval["labels"], "Test raw ESM proxy Hamiltonian", fig_dir / f"{stem}__test_raw_esm_H_hist.png")
